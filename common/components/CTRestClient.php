 ...
 */
class CTRestClient extends CComponent
{
    ...
    /**
     * Constructor
     * You can pass in the configuration for the CURL object here.
     *
     ...
     *   * string `server`: The REST server URL, like http://ipaddr/webservice/
     *   * boolean `http_auth` : Whether to authenticate using basic HTTP or not
     *   * string `http_user` : Username to authenticate, only needed if `http_auth` is TRUE
     *   * string `http_pass` : Password for authenticate.
     * @param array $config
     */
     ...
     * @param string $uri	: Relative URI resource on server.
     * @param array $params : Parameters to send as GET params
     * @param string $format: NULL, or json,xml,serialize,php,csv
     */
     ...
     * @param string $uri        : Relative URI resource on server.
     * @param array $params      : Parameters to send as POST params
     * @param string $format     : NULL, or json,xml,serialize,php,csv
     * @param boolean $isFilePost: whether or not files are posted along with the request
     */
    public function post($uri, $params = array(), $format = NULL, $isFilePost = false)
    {
        $curlMethodName = $isFilePost ? 'post_file' : 'post';
        return $this->_call($curlMethodName, $uri, $params, $format);
    }


    /**
     * Perform CURL PUT request
     * @param string $uri	: Relative URI resource on server.
     * @param array $params : Parameters to send as POST params
     * @param string $format: NULL, or json,xml,serialize,php,csv
     */

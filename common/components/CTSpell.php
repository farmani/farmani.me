...
class CTSpell extends CApplicationComponent
{
	...
	public function clean($string) {
		...
		foreach($string as $key => $word){
			...
			if(!empty($tmp) && strlen($tmp) > 1 && !in_array($tmp,$stopWord))
				$words[] = $this->singularize(strtolower($tmp));
		}

		return implode(' ',$words);
	}

	public function cleanRestaurantName($string) {
		$words = array();
		$stopWord = array('a', 'an', 'am', 'and', 'are', 'as', 'at', 'be', 'but', 'by', 'for', 'if', 'in', 'into', 'is', 'it', 'no','not','of','on','or','pm','s','such','t','that','the','their','then','there','these','they','this','to','was','will','with','restaurant','coffee',);
		//$string = preg_replace('/[^[:alpha:]]/', ' ',$string);
		$string = trim($string);
		$string = explode(" ", $string);
		foreach($string as $key => $word){
	...
	public function check($string) {
		...
		$query->add("spellcheck.colate",'false');
		$config = Yii::app()->params['solr.client'];
		$config['path'] = '/solr/core2';
		$client = new SolrClient($config);
		$client->setServlet(1,'spell');
		$query_response = $client->query($query);
		$response = $query_response->getResponse();
		$result = array();
		if(!empty($response['spellcheck']['suggestions'])){
			for($i=0;$i<count($response['spellcheck']['suggestions'])-4;$i++) {
				$suggests = array();
				foreach($response['spellcheck']['suggestions'][$i+1]['suggestion'] as $suggest)
					$suggests[] = $suggest['word'];
				$result[$response['spellcheck']['suggestions'][$i++]] = $suggests;
			}
		}
		return $result;
	}
	...
	public function add($string) {
		...
		$spellWord = $command->select('id, spell')
			->from('spell')
			->where('spell=:spell', array(':spell'=>$string))
			->queryRow();
		if($spellWord == false) {
			$command->reset();
			$command->insert('spell', array('spell'=>$string));
		}
	}
